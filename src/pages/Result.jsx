...
const SubHeader = ({ icon, title, className = "" }) => {
  ...
  return (
    <div
      className={`flex items-center gap-2 mb-3 sm:mb-4 mt-5 sm:mt-6 first:mt-0 ${className}`}
    >
...
const InningsReport = ({ team, score, wickets, overs, rr, num, children }) => (
  <div className="bg-[#0A0F1E]/60 border border-white/5 rounded-2xl sm:rounded-[24px] md:rounded-[32px] p-4 sm:p-6 md:p-10 backdrop-blur-xl relative overflow-hidden">
    ...
    <div className="relative z-10">
      <div className="flex justify-between items-end mb-5 sm:mb-8 border-b border-white/5 pb-4 sm:pb-6 gap-3">
        <div>
          <div className="text-[9px] sm:text-[10px] font-semibold uppercase tracking-wide text-indigo-400 mb-1.5 sm:mb-2">
            Innings {num}
          </div>
          <h2 className="text-xl sm:text-2xl md:text-5xl font-extrabold text-white uppercase tracking-tight">
            {team}
          </h2>
        </div>

        <div className="text-right shrink-0">
          <div className="text-2xl sm:text-3xl md:text-6xl font-extrabold text-white">
            {score}
...
export default function Result() {
  ...
  const StatusBadge = ({ outBy }) =>
    outBy ? (
      <span className="text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 sm:py-1 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-3xl">
        OUT
      </span>
    ) : (
      <span className="text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 sm:py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-3xl">
        NOT
      </span>
    );
  ...
  return (
    <div className="w-full max-w-[1300px] mx-auto pb-8 sm:pb-12">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-[24px] md:rounded-[32px] bg-[#0A0F1E] border border-white/[0.03] p-4 sm:p-6 md:p-12 text-center mb-5 sm:mb-8 shadow-xl">
        <div className="absolute inset-0 bg-gradient-to-b from-indigo-500/[0.02] to-transparent pointer-events-none"></div>

        <div className="relative z-10 flex flex-col items-center">
          {/* Top Label */}
          <div className="flex items-center gap-2 md:gap-3 mb-3 sm:mb-6">
            <div className="h-px w-6 md:w-8 bg-slate-800"></div>

            <span className="text-[11px] sm:text-sm font-semibold uppercase tracking-wider text-slate-400">
              Match Result
            </span>
          ...
          {/* Winner */}
          <div className="flex flex-col md:flex-row items-center justify-center gap-2 sm:gap-4 mb-3 sm:mb-6">
            <Trophy
            ...
            <div className="px-2 py-0.5 md:px-3 md:py-1 rounded bg-indigo-600 text-[11px] sm:text-sm font-semibold uppercase tracking-wide text-white shadow-lg">
              Winner
            </div>
          </div>

          {/* Sub Text */}
          <div className="inline-flex items-center gap-2 md:gap-3 px-3.5 py-1.5 sm:px-6 sm:py-2 rounded-lg md:rounded-xl bg-white/[0.02] border border-white/[0.05] backdrop-blur-md">
            <div className="h-1 w-1 md:h-1.5 md:w-1.5 rounded-full bg-emerald-500 animate-pulse"></div>

            <p className="text-[11px] sm:text-sm font-medium text-slate-300 uppercase tracking-wide">
              {subWinnerText}
            </p>
          </div>
        </div>
